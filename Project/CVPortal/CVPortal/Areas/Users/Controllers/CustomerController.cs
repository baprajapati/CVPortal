    ...
    public class CustomerController : Controller
    {
        ...
        public JsonResult EnableCustomer(int id)
        {
            try
            {
                ...
                var remarks = data.CustomerApprovals.Where(x => x.Status == "Rejected").OrderByDescending(x => x.CreatedByDate).FirstOrDefault()?.Remarks;
                ...
                Utility.SendMail(data.Email, CC1, BCC1, subject1, body1, displayName1, attachments1, true, Utility.UserId, EmailTypeEnum.Customer, id);
            ...
            try
            {
                ...
                data.ForEach(item =>
                {
                    ...
                    customers.Add(new CustomerListModel()
                    {
                        ...
                        RejectedReason = item.CustomerApprovals.Where(x => x.CustomerId == item.ID && x.Status == "Rejected").OrderByDescending(x => x.CreatedByDate).FirstOrDefault()?.Remarks,
